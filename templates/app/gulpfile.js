 ...
 * Static file server
 */
gulp.task('statics', staticFileServer);
 ...
 * Static file server
 */
function staticFileServer(dist) {
    connect.server({
        root: (dist ? ['./dist'] : ['./.tmp', './.tmp/src/app', './src/app', './bower_components']),
        port: (dist ? 4000 : 3000),
        livereload: true,
        <%
        if (middleware) { %>
...
gulp.task('watch', ['statics', 'default'], function() {
    ...
    gulp.watch(['./src/app/**/*.<%= styleData.extension %>'], ['csslint']);
});
